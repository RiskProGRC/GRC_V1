...
- 2026-07-02: QA task started (facade/dead-UI remediation). QA1 (Dashboard real-data) COMPLETE on `feat/qa-dashboard-real-data` (stacked on IA). Replaced 9 hardcoded dashboard widgets with live DB queries: heatmap (assessment grid), residual-by-category gauges (real riskcat + avg residual), aggregate exposure (inherent/residual/target score sums), proportional pipeline bar widths, ownership map (risk GROUP BY dept), treatment distribution doughnut (assessment.treatment), untreated-risk watchlist, top-priority table (highest residual). KRI radar renders real KRI or honest empty-state. Zero console errors. Screenshots screenshots/qa1_dashboard/.
  Button audit (agent) produced a verified list of ~35 pages with dead/partial action buttons — to be fixed in QA2+ phases: blank-table var bugs (kpi/recommendations/kra_settings), literal-id delete bugs (risklist/recommendations/kpi/actions), class mismatch (userslist delete-user), missing endpoints (companylist editcompanylist, entity.php adddepartment, reviewer riskEdit), native-submit blank-JSON pages (nominee/owner/project/permissionedit/bussinf group), riskstatus.php missing triggers.
- 2026-07-02: QA2 (action-button fixes) COMPLETE on `feat/qa-action-button-fixes`. Fixed 5 CONFIRMED broken delete buttons (verified each against code — audit had false positives on "blank table" claims which were actually header.php-provided vars + empty data, NOT bugs): userslist delete-user→user-delete class match (+data-name); risklist/recommendations/kpi literal-`.$id.` in raw HTML → `<?=id?>`; actions empty id → real id. Verified end-to-end: buttons render real ids (risklist 198/197/196, actions 1/2/3) and modal-populate endpoints return real records. Screenshots screenshots/qa2_buttons/.
